 ...
 * Widget 的 [StarBurstWidgetProvider.onUpdate] 在桌面进程被拉起时同步读取，
 * 不依赖应用内存态。
 *
 * @author Hsi Chu
 * @since V1.3.0
...
class WidgetSnapshotStore @Inject constructor(
    ...
    /** 触发系统回调 [StarBurstWidgetProvider.onUpdate]，让 Widget 重新渲染。 */
    fun requestUpdate() {
        ...
        fun requestUpdate(context: Context) {
            runCatching {
                ...
                val ids = manager.getAppWidgetIds(
                    ComponentName(context, StarBurstWidgetProvider::class.java),
                )
                if (ids.isEmpty()) return
                val intent = Intent(context, StarBurstWidgetProvider::class.java).apply {
