...
class ServerAsrRecorder(
        ...
        try {
            ...
            if (stopRequested && !aborted) {
                ...
                if (final != null && final.isNotBlank()) {
                    // 引擎 finish 返回的累积文本偶尔会把整段内容带重复
                    // （“句子 句子”），就地折叠成一份，避免松手上屏后文本显示两遍。
                    val cleanFinal = collapseDuplicatedSegment(final)
                    withContext(Dispatchers.Main) { l.onPartialResult(cleanFinal) }
                    // 流式识别常有重字与同音错字，交给大模型就地校对。放到后台跑：
                    // 不阻塞 stop() 返回，校正好就直接替换输入框里的文字。后端未配
                    // LLM、调用失败或校对结果被判定不可信时返回原文，等于跳过这一步。
                    val gen = generation
                    refineJob = scope.launch {
                        val polished = api.refine(backendUrl, backendToken, cleanFinal)
                        val cleanPolished = collapseDuplicatedSegment(polished.orEmpty())
                        // 用户可能已经开始了下一段录音，那就别覆盖新内容。
                        if (generation == gen && cleanPolished.isNotBlank() && cleanPolished != cleanFinal) {
                            withContext(Dispatchers.Main) { l.onPartialResult(cleanPolished) }
                        }
                    }
                }
            }
    ...
    /**
     * 折叠「整段重复」的转写文本：若文本恰好由两段相同内容拼接（“句子 句子”），
     * 只保留前半。引擎 finish 的累积结果偶发整段重复，这里做兜底，不改变单份语义。
     */
    private fun collapseDuplicatedSegment(text: String): String {
        if (text.isBlank() || text.length < 2) return text
        val even = text.length % 2 == 0
        val half = text.length / 2
        if (even && text.take(half) == text.drop(half)) return text.take(half)
        // 容错：中间可能有分隔空格/标点（“X X” 或 “X。X”），按非空段切分再比。
        val trimmed = text.trim()
        if (trimmed.length < 2) return text
        val tHalf = trimmed.length / 2
        if (trimmed.take(tHalf).trim() == trimmed.drop(tHalf).trim()) return trimmed.take(tHalf).trim()
        return text
    }
