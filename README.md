...
- **Project Overview** — a chat ⋮ menu entry that scans the session's project directory and shows file/line statistics by type (code/comment/blank), the largest files, and Git repository info (branch, commit count, last commit, modified files and diffstat)
...
- **Accent color & theme schemes** — eleven brand accent palettes plus five full theme schemes
  (**Candy / Ocean / Sunset / Flame / Bubble**) that recolor the whole app, applied app-wide
  (light/dark/AMOLED) and synced
...
- **Model & agent control** — search providers/models, cycle agents, view token usage and context.
  The chat top bar shows the session's **cumulative token usage and cost**, while the budget ring
  and context-usage dialog show the **current context occupancy** (estimated tokens / model window)
  — two distinct figures, no more misread cumulative tokens
