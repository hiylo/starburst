...
- **端侧建议** — 通过内置 MNN 模型（Qwen3.5-0.8B）**完全离线**生成下一步建议，可选配**云端 LLM 服务**以获得更高质量。权重在首次使用时从 **ModelScope**（国内访问快）逐个文件下载，并对每个文件做 SHA-256 校验；只有预先放入 `app/src/main/assets/models/` 的发布构建才改为从 APK 内自动解压。配置后端后，建议优先走后端 LLM，其次外部服务，最后端侧模型，并显示来源标签
...
- **项目概览** — 对话页 ⋮ 菜单入口，扫描会话项目目录，按类型展示文件/行数统计（代码/注释/空行）、最大的文件，以及 Git 仓库信息（分支、提交数、最近提交、改动文件数与 diffstat）
- **服务器与服务商管理** — 服务信息（版本/活跃会话）、CPU/内存/磁盘、配置查看与修改、重启，以及自定义服务商管理（添加/编辑/删除服务商与模型）
- **文件编辑** — 应用内编辑并保存工作区文件
- **对话总结** — 消息级或整个会话的总结（云端 LLM，回退端侧模型），流式展示
- **自定义强调色与主题方案** — 十一种品牌强调色，外加五套完整主题方案（**糖果/海洋/日落/火焰/泡泡**）整体换肤，全局生效（亮/暗/AMOLED）并支持设置同步
...
1. 在首页点击 **+**
2. 输入服务器地址（例如 `http://192.0.2.10:4096`）、用户名和可选密码
3. 点击 **连接**
...
> 发布构建会链接 MNN 原生库，因此首次构建需要几分钟。`app/src/main/assets/models/` 下的模型权重已在 `.gitignore` 中且从不入库：干净克隆构建出的包同样正常工作，首次使用时从 **ModelScope** 下载模型（约 520 MB）。只有构建前预先放入该目录，权重才会打进 APK。
